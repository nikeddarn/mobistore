<div id="vendorShipments" class="m-b-4">
    <table class="table table-responsive">
        ...
        <tr>
            <td colspan="4">
                <strong>Неотправленная отгрузка:&nbsp;</strong>
                <span>{{ $shipment->vendorShipment->vendorCourier->name }}
                    ,&nbsp;{{ $shipment->planned_departure->format('d-m-Y') }}.&nbsp;</span>
                <span>Сумма:&nbsp;<strong>${{ $shipment->invoice->sum('invoice_sum') }}</strong></span>
            </td>
        </tr>

        @if($shipment->invoice->count())
            ...
            <td colspan="4">
                <div class="row m-t-2">
                    ...
                    <div class="col-xs-6 text-right">
                        <form method="post" action="{{ route('vendor.shipment.dispatch') }}">
                            {{ csrf_field() }}
                            <input type="hidden" name="shipments_id" value="{{ $shipment->id }}">
                            <button type="submit" class="btn btn-primary"><i class="fa fa-long-arrow-right"></i>Отправить
                            </button>
                        </form>
                    </div>
                    @endif
